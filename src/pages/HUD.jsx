...
export default function HUD() {
  ...
  const [is blurred, setBlurred] = useState(false);

  // ---------------------------------------------------------
  // 1. HARDWARE & IP LOCK (The Chromebook Check)
  // ---------------------------------------------------------
  useEffect(() => {
    const checkAccess = async () => {
        const ua = navigator.userAgent;
        // RESTORED: Strict check for ChromeOS or Linux
        const isChromebook = ua.includes("CrOS") || ua.includes("Linux");

        if (!isChromebook) {
            // Instant redirect for non-Chromebooks
        ...
        try {
            ...
            if (res.ok) setVerificationStatus("ACCESS_GRANTED");
            else {
                setVerificationStatus("UNAUTHORIZED_IP");
                setTimeout(() => window.location.href = "/", 2000);
            }
        } catch(e) { setVerificationStatus("NET_ERROR"); }
    };
    checkAccess();
  }, []);

  // ---------------------------------------------------------
  // 2. PRIVACY VEIL (Anti-Shoulder Surfing)
  // ---------------------------------------------------------
  useEffect(() => {
  ...
  // ---------------------------------------------------------
  // 3. PANIC KEY (Double ESC to Logout)
  // ---------------------------------------------------------
  useEffect(() => {
      let lastPress = 0;
  ...
  useEffect(() => {
    ...
    const hbInt = setInterval(sendHeartbeat, 30000); // Every 30s

    // Send "I am leaving" signal on close
    const cleanup = () => {
        navigator.sendBeacon('/api/stream_keepalive', JSON.stringify({ password, task, status: false }));
    };
      ...
      <div className="min-h-screen bg-black flex items-center justify-center p-4 font-mono">
        <div className="w-full max-w-md border border-cyan-900/50 p-8 bg-slate-950 text-center shadow-[0_0_30px_rgba(8,145,178,0.2)]">
          ...
          <form onSubmit={async (e) => {
            ...
            <input type="password" value={password} onChange={e=>setPassword(e.target.value)} className="w-full bg-black border border-slate-800 p-3 text-cyan-400 text-center outline-none focus:border-cyan-500" autoFocus placeholder="ENTER PASSCODE"/>
            {error && <div className="text-red-500 text-xs mt-4 font-bold">{error}</div>}
            <button type="submit" disabled={loading} className="w-full mt-6 bg-cyan-900/20 text-cyan-400 py-2 border border-cyan-900 hover:bg-cyan-900/40 font-bold tracking-widest">
  ...
  return (
    <div className={`min-h-screen font-mono p-6 overflow-hidden flex flex-col transition-colors duration-300 relative ${threatLevel === 'CRITICAL' ? 'bg-red-950' : 'bg-black text-green-500'}`}>
      ...
      {/* PRIVACY VEIL OVERLAY */}
      {is blurred && (
          <div className="absolute inset-0 z-50 bg-black/90 backdrop-blur-xl flex flex-col items-center justify-center text-slate-500">
              <EyeOff className="w-16 h-16 mb-4" />
              <p className="tracking-widest">VISUAL_FEED_SUSPENDED</p>
          </div>
      )}
      ...
      <div className={`flex justify-between items-end border-b pb-4 mb-6 ${threatLevel === 'CRITICAL' ? 'border-red-500' : 'border-green-900/50'}`}>
        <div>
            ...
            <div className="flex items-center gap-2 text-xs mt-1">
                ...
                <span className="ml-4 opacity-50">DEVICE: CHROMIUM_OS</span>
            </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-6 flex-1">
        <div className="col-span-1 space-y-6">
            <div className={`border p-4 ${threatLevel === 'CRITICAL' ? 'border-red-500 bg-red-900/20' : 'border-green-900 bg-green-950/5'}`}>
                ...
                <textarea value={task} onChange={e => setTask(e.target.value)} className="w-full bg-black border border-slate-800 p-3 text-xl text-white outline-none focus:border-green-500 h-32 resize-none"/>
            </div>
            <div className={`border p-4 ${threatLevel === 'CRITICAL' ? 'border-red-500 bg-red-900/20' : 'border-green-900 bg-green-950/5'}`}>
        ...
        <div className={`col-span-2 border p-4 bg-black relative overflow-hidden flex flex-col ${threatLevel === 'CRITICAL' ? 'border-red-500' : 'border-green-900'}`}>
            ...
            <div className="space-y-2 font-mono text-sm flex-1 overflow-y-auto custom-scrollbar relative z-10">
                {logs.map((log, i) => (
                    <div key={i} className={`flex gap-4 border-b pb-1 ${log.actor_type === 'ATTACKER' ? 'text-red-500 border-red-900/50' : 'text-green-500 border-green-900/20'}`}>
