...
export default function HUD() {
  ...
  const handleLogin = async (e) => {
    ...
    setError('');
    try {
        // RENAMED: heartbeat -> stream_keepalive
  ...
  useEffect(() => {
    const checkAccess = async () => {
        ...
        if (!isSafeDevice) { window.location.href = "https://www.youtube.com/watch?v=dQw4w9WgXcQ"; return; }
        try {
            // RENAMED: hud-access -> client_verify
            const res = await fetch('/api/client_verify');
            if (res.ok) setVerificationStatus("ACCESS_GRANTED");
            else setVerificationStatus("UNAUTHORIZED_IP (LOGIN REQUIRED)");
  ...
  if (verificationStatus === "NET_ERROR") return <div className="min-h-screen bg-black text-red-600 font-mono flex items-center justify-center text-xl animate-pulse">NETWORK FAILURE</div>;
  ...
  return (
    <div className={`min-h-screen font-mono p-6 overflow-hidden flex flex-col transition-colors duration-300 ${threatLevel === 'CRITICAL' ? 'bg-red-950' : 'bg-black text-green-500'}`}>
      <div className={`flex justify-between items-end border-b pb-4 mb-6 ${threatLevel === 'CRITICAL' ? 'border-red-500' : 'border-green-900/50'}`}>
        <div>
            <h1 className={`text-4xl font-bold tracking-tighter ${threatLevel === 'CRITICAL' ? 'text-red-500 animate-pulse' : 'text-white'}`}>{threatLevel === 'CRITICAL' ? '⚠️ SECURITY ALERT ⚠️' : 'COMMAND_STATION'}</h1>
            <div className="flex items-center gap-2 text-xs mt-1"><Wifi className="w-3 h-3 animate-pulse" /><span>UPLINK_ESTABLISHED</span></div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-6 flex-1">
        ...
        <div className={`col-span-2 border p-4 bg-black relative overflow-hidden flex flex-col ${threatLevel === 'CRITICAL' ? 'border-red-500' : 'border-green-900'}`}>
            ...
            <div className="space-y-2 font-mono text-sm flex-1 overflow-y-auto custom-scrollbar relative z-10">
                {logs.map((log, i) => (
                    <div key={i} className={`flex gap-4 border-b pb-1 ${log.actor_type === 'ATTACKER' ? 'text-red-500 border-red-900/50' : log.actor_type === 'ADMIN' ? 'text-yellow-500 border-yellow-900/30' : 'text-green-500 border-green-900/20'}`}>
                        <span className="opacity-50">{new Date(log.timestamp).toLocaleTimeString()}</span><span className="w-20 font-bold">{log.actor_type}</span><span className="w-32">{log.action}</span><span className="truncate flex-1 opacity-80">{log.details}</span>
                    </div>
                ))}
