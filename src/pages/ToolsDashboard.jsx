...
import { 
  ...
  Activity, Globe, Eye, MapPin, AlertOctagon, Box, ExternalLink
} from 'lucide-react';
...
// ==========================================
const fieldKitTools = [
  { name: "Shodan", url: "https://www.shodan.io/", desc: "Search engine for Internet-connected devices.", icon: Globe },
  { name: "CyberChef", url: "https://gchq.github.io/CyberChef/", desc: "The Cyber Swiss Army Knife.", icon: Box },
  { name: "HaveIBeenPwned", url: "https://haveibeenpwned.com/", desc: "Check compromised emails.", icon: Shield },
  { name: "Wayback Machine", url: "https://archive.org/web/", desc: "View internet history.", icon: Radio },
  { name: "URLScan.io", url: "https://urlscan.io/", desc: "Analyze malicious URLs.", icon: Search },
  { name: "Exploit-DB", url: "https://www.exploit-db.com/", desc: "Archive of exploits.", icon: AlertTriangle },
];
function FieldKit() {
  return (
      ...
      {fieldKitTools.map((tool) => (
        <a key={tool.name} href={tool.url} target="_blank" rel="noopener noreferrer" className="block group">
          <Card className="bg-slate-900/50 border-slate-800 hover:border-cyan-500/50 transition-all h-full">
            <CardHeader className="pb-2"><CardTitle className="text-slate-200 flex items-center justify-between text-base"><span className="flex items-center gap-2"><tool.icon className="w-4 h-4 text-cyan-500" /> {tool.name}</span><ExternalLink className="w-3 h-3 text-slate-600 group-hover:text-cyan-400" /></CardTitle></CardHeader>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8 font-sans pb-24">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8"><h1 className="text-3xl md:text-4xl font-mono font-bold mb-4 flex items-center gap-3"><Server className="w-8 h-8 text-cyan-500" /><span className="text-slate-100">Cyber Army Knife <span className="text-cyan-600 text-sm align-top">v3.3</span></span></h1><p className="text-slate-400">Integrated with OpenAI, VirusTotal, and Global OSINT Feeds.</p></div>
        ...
        <Tabs defaultValue={defaultTab} className="space-y-6">
          ...
          <TabsContent value="tools" className="space-y-6"><div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6"><Card className="bg-slate-900/50 border-slate-800"><CardHeader><CardTitle className="text-red-400 flex items-center gap-2"><Terminal className="w-5 h-5"/> Reverse Shell Gen</CardTitle></CardHeader><CardContent><ReverseShellGen /></CardContent></Card><Card className="bg-slate-900/50 border-slate-800"><CardHeader><CardTitle className="text-green-400 flex items-center gap-2"><Lock className="w-5 h-5"/> "Ghost Writer" AES</CardTitle></CardHeader><CardContent><AesEncryptor /></CardContent></Card><Card className="bg-slate-900/50 border-slate-800"><CardHeader><CardTitle className="text-purple-400 flex items-center gap-2"><Eye className="w-5 h-5"/> Steganography Studio</CardTitle></CardHeader><CardContent><SteganographyTool /></CardContent></Card></div></TabsContent>
