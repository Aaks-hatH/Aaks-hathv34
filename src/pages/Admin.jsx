...
export default function Admin() {
  ...
  // --- 1. SECURE DATA SYNC (The Fix) ---
  const refreshSecureData = async () => {
    if (!password) return; // Need password to use the secure tunnel

    try {
        // Fetch Banned IPs via Proxy
  ...
  const refreshPublicData = async () => {
    // Guestbook is public, so we can fetch it directly
  ...
  useEffect(() => {
    ...
    // A. LIVE VISITORS (Ephemeral data, okay to use Realtime)
    ...
    // B. POLL SECURE DATA (Since we remove public RLS, we can't use Realtime for logs easily)
    refreshPublicData();
    refreshSecureData();
    const interval = setInterval(refreshSecureData, 5000); // Poll every 5s
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await fetch('/api/auth_handshake', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ password, token: totp, captcha: turnstileToken })
      });
  ...
  const banUser = async (ip) => {
    ...
    setTimeout(refreshSecureData, 1000); // Force refresh
  };
        ...
        {activeTab === 'LIVE_TRAFFIC' && (
          <div className="space-y-6">
            <div className="border border-slate-800 bg-slate-950 rounded overflow-hidden relative z-0">
               ...
               <MapContainer key={activeTab} center={[20, 0]} zoom={2} style={{ height: '400px', width: '100%', background: '#020617' }}>
                  ...
                  {Object.values(visitors).map((v) => {
                    ...
                    return (
                      <Marker key={u.id} position={[lat, lon]} icon={customMarkerIcon}>
                         <Popup className="font-mono text-xs text-black"><div className="font-bold">{u.ip}</div><div>{u.geo}</div><div className="text-[10px] text-slate-500">{u.path}</div></Popup>
                      </Marker>
                    );
                  })}
               </MapContainer>
