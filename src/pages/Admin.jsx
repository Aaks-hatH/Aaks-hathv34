...
export default function Admin() {
  ...
  const handleLogin = async (e) => {
    ...
    } catch (err) { setError('CONNECTION_ERROR'); } finally { setLoading(false); }
  };
      ...
      <div className="min-h-screen bg-black text-slate-400 font-mono p-4 flex flex-col items-center justify-center">
        <div className="w-full max-w-md border border-slate-800 p-8 bg-slate-950 shadow-[0_0_50px_rgba(0,0,0,0.5)]">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            {!stepUpRequired && (
              <>
                <div className="space-y-1"><label className="text-[10px] uppercase text-slate-600">Identification</label><input type="password" value={password} onChange={e=>setPassword(e.target.value)} className="w-full bg-black border border-slate-700 p-2 text-white focus:border-cyan-500 outline-none" placeholder="PASSWORD" /></div>
                <div className="space-y-1"><label className="text-[10px] uppercase text-slate-600">Auth Token</label><input type="text" value={totp} onChange={e=>setTotp(e.target.value)} maxLength={6} className="w-full bg-black border border-slate-700 p-2 text-white tracking-widest focus:border-cyan-500 outline-none" placeholder="000000" /></div>
              </>
            )}
            {stepUpRequired && (
                <div className="space-y-1 animate-in slide-in-from-right"><label className="text-[10px] uppercase text-yellow-500 font-bold">Verification Code</label><input type="text" value={stepUpCode} onChange={e=>setStepUpCode(e.target.value)} maxLength={6} className="w-full bg-yellow-950/20 border border-yellow-600 p-2 text-yellow-400 tracking-widest focus:border-yellow-400 outline-none text-center text-xl font-bold" placeholder="______" autoFocus /></div>
            )}
            <div className="flex justify-center my-2"><Turnstile siteKey="0x4AAAAAACFDgNVZZRDkdRXG" onSuccess={(token) => setTurnstileToken(token)} options={{ theme: 'dark' }} /></div>
            {error && <div className="text-red-500 text-xs bg-red-950/20 p-2 border-l-2 border-red-500 font-bold">{error}</div>}
            <button type="submit" disabled={loading} className={`w-full py-2 text-xs font-bold transition-colors ${stepUpRequired ? 'bg-yellow-600 text-black hover:bg-yellow-500' : 'bg-slate-100 text-black hover:bg-cyan-500 hover:text-white'}`}>{loading ? "PROCESSING..." : stepUpRequired ? "VERIFY" : "ACCESS"}</button>
          </form>
        </div>
      </div>
    );
